...
{
    public partial class ComponentBrand : UserControl
    {
        private readonly BrandService _brandService = new BrandService();
        private readonly AccountService _accountService = new AccountService();
        private BrandLayout _layout =  new BrandLayout();
        ...
        private Panel home;
        private BrandLayout brandLayout;
        private Brand item;

        public ComponentBrand(Panel home, BrandLayout layout, Brand brand)
        {
            InitializeComponent();
            this._home = home;
            this._layout = layout;
        ...
        private void LoadPreImage()
        {
            ...
            this.originalImage = (Image)rm.GetObject("brand (1)");
        }

        private async void LoadData(Brand brand)
        {
            var user = await this._accountService.GetById(brand.CreateUserId);
            this.item_name.Text = brand.Name;
            this.item_des.Text = brand.Description;
            this.item_date.Text = brand.CreatedAt?.ToString("dd/MM/yyyy hh:mm tt", CultureInfo.CreateSpecificCulture("en-US"));
            this.item_user.Text = user.Code == 0 ? user.Data.Name : "Không có thông tin";
           
        }
        private void ComponentBrand_Load(object sender, EventArgs e)
        {
        ...
        private void btn_edit_Click(object sender, EventArgs e)
        {
            ...
            this._home.Controls.Add(new BrandLayout(this._home, 2, this._brand.Id));
        }

        
        private async void btn_delete_Click(object sender, EventArgs e)
        {
            var dialogRs = MessageBox.Show("Bạn có chắc chắn muốn xóa thương hiệu này?","Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            if(dialogRs == DialogResult.OK)
            {
                var result = await _brandService.Delete(this._brand.Id);
                if (result.Code == 0)
                {
        ...
        {
            this._home.Controls.Clear();
            this._home.Controls.Add(new BrandLayout(this._home, 3, this._brand.Id));
        }
