...
{
    public partial class ComponentExported : UserControl
    {
        ...
        public ComponentExported(Panel home, ExportLayout layout, string id, bool allowAction = true)
        {
            InitializeComponent();
            this._home = home;
            this._layout = layout;
            ...
            this._orderDetail = ExportDetailGlobal.SelectedItems.FirstOrDefault(s => s.ProductId == id);
            this.LoadData(id);
            this.ControlAction();
        }
        ...
        {
            var exportItem = ExportGlobal.SelectedItems.FirstOrDefault(s => s.SelectedProduct.Id == this._productId);
            ExportGlobal.SelectedItems.Remove(exportItem);
            var exportDetailItem = ExportDetailGlobal.SelectedItems.FirstOrDefault(s => s.ProductId == this._productId);
            ExportDetailGlobal.SelectedItems.Remove(exportDetailItem);
        ...
        private void priceIn_txt_TextChanged(object sender, EventArgs e)
        {
            ...
            var exportDetailF = ExportDetailGlobal.SelectedItems.FirstOrDefault(s => s.ProductId == this._productId);
            //exportDetailF.PriceOut = value;
            exportDetailF.Total = exportDetailF.Quantity * value;
        }

        private int previousValue = 0;
        private async void quantity_num_ValueChanged(object sender, EventArgs e)
        {
            var value = int.Parse(this.quantity_num.Value.ToString());
            var exportDetailF = ExportDetailGlobal.SelectedItems.FirstOrDefault(s => s.ProductId == this._productId);
            var productRS = await this._productService.GetById(this._productId); 
            if(productRS.Code == 0)
            {
                var product = productRS.Data;
                if(value > product?.Quantity)
                {
                    MessageBox.Show($"Số lượng vượt quá sản phẩm trong kho\n{product?.Name}");
                    quantity_num.Value = previousValue;
                    return;
                }
            }
            exportDetailF.Quantity = value;
            exportDetailF.Total = (exportDetailF.Product.Price*exportDetailF.Product.Discount)* value;
            previousValue = value;
        }
